...
import { completeCommunityOnboarding, createLocalReview, getCurrentIdentity, submitReviewForModeration, type ReviewSyncStatus } from "../lib/review-data";
...
function ReviewRoute({ identity, targets, unavailableMessage, onAuth, onSubmit }: { identity: ReviewIdentity | null; targets: UniversityTarget[]; unavailableMessage: string | null; onAuth: () => void; onSubmit: (draft: ReviewDraft) => Promise<{ ok: true; status: ReviewSyncStatus } | { ok: false; message: string }> }) { const [params] = useSearchParams(); const initialUniversityId = params.get("university"); return <ReviewWizard identity={identity} universities={targets} unavailableMessage={unavailableMessage} {...(initialUniversityId ? { initialUniversityId } : {})} onRequireAuth={onAuth} onSubmit={onSubmit} />; }
...
export function PortalRoutes() {
  ...
  const [universities, setUniversities] = useState<University[]>([]); const [directoryMessage, setDirectoryMessage] = useState<string | null>("Loading the MQA institution catalogue…");
  const [targets, setTargets] = useState<UniversityTarget[]>([]); const [targetMessage, setTargetMessage] = useState<string | null>("Loading the MQA institution catalogue…"); const [saved, setSaved] = useState<string[]>([]); const [compare, setCompare] = useState<string[]>([]);
  useEffect(() => {
    void getCurrentIdentity().then(setIdentity);
    void loadCommunityDirectory().then((result) => { setUniversities(result.universities); setTargets(result.reviewTargets); setTargetMessage(result.reviewTargets.length ? null : "The institution catalogue is temporarily unavailable."); setCloudReviews(result.reviews); setDirectoryMessage(result.message); });
