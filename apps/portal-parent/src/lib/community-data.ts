...
export async function loadCommunityDirectory(): Promise<{ universities: University[]; reviewTargets: UniversityTarget[]; reviews: Review[]; connected: boolean; message: string | null }> {
  try {
    ...
    for (const row of programmeRows) {
      if (!row.reference_institution_id || !row.qualification_name) continue;
      const current = programmes.get(row.reference_institution_id) ?? [];
      if (!current.includes(row.qualification_name)) current.push(row.qualification_name);
      programmes.set(row.reference_institution_id, current);
    }
    ...
    const universities = universityRows.map((row) => {
      ...
      return {
        ...
        strengths: relevantReviews.map((review) => review.greenFlags).filter(Boolean).slice(0, 3), weaknesses: relevantReviews.map((review) => review.redFlags).filter(Boolean).slice(0, 3), courses: programmes.get(row.reference_institution_id) ?? [],
    ...
    const partial = programmesResult.error || summariesResult.error || reviewsResult.error;
    return { universities, reviewTargets, reviews, connected: !partial, message: partial ? "Some programme or community evidence is temporarily unavailable." : null };
  } catch { return { universities: [], reviewTargets: [], reviews: [], connected: false, message: "The university directory is temporarily unavailable." }; }
}
