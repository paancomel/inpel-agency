...
interface Props {
  identity: ReviewIdentity | null; universities: UniversityTarget[]; initialUniversityId?: string; unavailableMessage?: string | null;
  onRequireAuth: () => void; onSubmit: (draft: ReviewDraft) => Promise<{ ok: true; status: ReviewSyncStatus } | { ok: false; message: string }>;
}

export function ReviewWizard({ identity, universities, initialUniversityId, unavailableMessage, onRequireAuth, onSubmit }: Props) {
      ...
      <div className="review-shell">
        ...
        <section className="wizard-content">
          ...
          <div className="wizard-body">
            ...
            {step === 5 ? <div><h2 ref={headingRef} tabIndex={-1} className="wizard-title">Anonymous by design.</h2><p className="wizard-lede">Review the essentials and confirm before your submission enters moderation.</p><div className="review-summary"><div><span>Institution</span><strong>{universities.find((item) => item.id === draft.universityId)?.name}</strong></div><div><span>Course · year</span><strong>{draft.course} · {draft.year}</strong></div><div><span>Total rating</span><strong>{totalScore}/10</strong></div><div><span>Contribution</span><strong>Standard review</strong></div></div><fieldset className="declarations"><legend>Required confirmations</legend><label><input type="checkbox" checked={draft.declarations.terms} onChange={(e) => setDraft({ ...draft, declarations: { ...draft.declarations, terms: e.target.checked } })} />I agree to the <Link to="/legal/terms">Terms &amp; Conditions</Link>.</label><label><input type="checkbox" checked={draft.declarations.privacy} onChange={(e) => setDraft({ ...draft, declarations: { ...draft.declarations, privacy: e.target.checked } })} />I acknowledge the <Link to="/legal/privacy">Privacy Policy</Link> and automated safety processing.</label>{([ ["age", "I declare that I am 18 years old or older."], ["rights", "This content is mine to submit and safe for anonymous publication."] ] as const).map(([key, label]) => <label key={key}><input type="checkbox" checked={draft.declarations[key]} onChange={(e) => setDraft({ ...draft, declarations: { ...draft.declarations, [key]: e.target.checked } })} />{label}</label>)}</fieldset><div className="notice-card"><ShieldCheck /><p><strong>Your public identity stays anonymous.</strong><br />Your account link, declarations, and submission time are retained privately for safety, moderation, and compliance. They are never published.</p></div></div> : null}
          ...
          {step <= 5 ? <footer className="wizard-actions">{step > 1 ? <button type="button" className="button-ghost" onClick={() => setStep(step - 1)}><ArrowLeft size={18} />Back</button> : <a className="button-ghost" href="/">Cancel</a>}{step < 5 ? <button type="button" className="button-primary" disabled={step === 1 && !universities.length} onClick={goNext}>Continue<ArrowRight size={18} /></button> : <button type="button" className="button-primary" disabled={busy} onClick={() => void submit()}>{busy ? "Submitting…" : identity ? "Submit for review" : "Sign in & submit"}<ArrowRight size={18} /></button>}</footer> : null}
        </section>
      </div>
    </main>
  );
