...
describe("parent invitation routing", () => {
  ...
  async function completePriorities(user: ReturnType<typeof userEvent.setup>) {
  ...
  async function authenticateParent(user: ReturnType<typeof userEvent.setup>) {
    ...
    await screen.findByRole("button", { name: /continue as this parent/i });
  }
  ...
  it("offers every Malaysian state and creates a durable invitation", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /continue as this parent/i }));
  ...
  it("redirects an invalid session route back to the parent portal", async () => {
    ...
    expect(await screen.findByRole("heading", { name: /family priorities/i }, { timeout: 5_000 })).toBeInTheDocument();
  });
